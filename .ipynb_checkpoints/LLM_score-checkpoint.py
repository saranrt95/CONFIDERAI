...
def ComputeNewScores(X, parsedruleset, featurelabels,relevances,verifiedidx, changeclsidx, rulesimilarity):
...
def ComputeLLMScores(calibrationsetfile,parsedruleset,relevances,outputlabel,featurelabels,changeclsidx,cls0label,cls1label,resultsfile,rulesimilarity,save_result=False):
	...
	scores0,scores1,tauvalues= ComputeNewScores(X, parsedruleset,featurelabels, relevances, verifiedidx,changeclsidx, rulesimilarity)
	X = X.copy()
